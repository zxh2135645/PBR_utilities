...
if __name__ == '__main__':
    ...
    for msid in ms_list:
        ...
        if len(lst_edit_check) == 0:
            status.loc[status['msid'] == msid, 'mse'] = 'NA'
            print("lst_edits files do not have edited lesion in the mindcontrol for msid:",
                             "\n {0}".format(msid))
        else:
            status.loc[status['msid'] == msid, 'mse'] = mse
